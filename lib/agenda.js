...
Agenda.prototype.database = function(url, collection, options) {
  ...
  this._db.ensureIndex("nextRunAt", ignoreErrors)
          ...
          .ensureIndex("jobTag", ignoreErrors)
          ;
...
Agenda.prototype.create = function(name, data) {
  ...
  var jobprop = {name: name, jobTag: data.jobTag, removeOnDone: data.removeOnDone ? true : false, data: data,
    type: 'normal', priority: priority, agenda: this};
  delete data.jobTag;
  delete data.removeOnDone;
  var job = new Job(jobprop);
  return job;
};
